...
export function useOpenFeature() {
  ...
  const getObjectDetails = useCallback(
    async (flagKey: string, defaultValue: Record<string, string>, context?: Record<string, unknown>) => {
  ...
  const track = useCallback(
    async (trackingEventName: string, context?: Record<string, unknown>, details?: Record<string, unknown>) => {
      await waitForOpenFeature();
